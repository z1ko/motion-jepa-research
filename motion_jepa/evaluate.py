...
def run_linear_probe(
    *,
    ...
    """Leave-one-subject-out linear probe. Returns per-fold and aggregate metrics."""
...
def print_report(results: list[dict]) -> None:
    ...
    print("=" * 78)
    header = f"{'dataset':<12} {'label':<8} {'windows':>8} {'classes':>8} {'folds':>6} {'acc':>8} {'bal.acc':>8} {'chance':>8} {'majority':>9}"
    print(header)
    print("-" * len(header))
    for r in results:
        print(
    ...
    print("=" * 78)
    print(
        "acc/bal.acc are held-out-subject linear-probe accuracy; chance = 1/n_classes; "
